...
\end{aligned}
$$
Thus, we may proceed to maximum likelihood estimation via profile likelihood as above, using the `Vtype = diag` argument to `lmn.suff()` to optimize calculations for diagonal $\VV$.
...
- Given a prior distribution
	...
	The marginal posterior distribution $p(\tth \mid \YY)$ has a closed-form expression calculated by `lmn.marg`.
...
$$
which is obtained from `lmn.prior()`:

```{r gcir_prior}
prior <- lmn.prior(p = 2, q = 1) # default prior
prior
```
